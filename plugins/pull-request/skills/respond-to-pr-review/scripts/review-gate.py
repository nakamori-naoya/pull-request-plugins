...
承認範囲の照合は同じdirectoryの approval.py が行う（actions に pull-request/ で始まる確認の名前、pull_requests にPR番号）。
...
def main():
    ...
    if required:
        if args.approval is None:
            ...
            outside = approval_scope.mismatch(scope, action, args.pr, None, datetime.now(timezone.utc))
