...
"""公開Git操作の外にある人の確認を、承認範囲に照らして通すか止める。
  ...
  python3 scripts/gate.py --action <確認の名前> --branch <作業branch> [--approval '<承認範囲のJSON>']
...
def main() -> None:
    ...
    if extra or not args.action or not args.branch:
        emit({"status": "invalid", "reason": "action_and_branch_required"}, 2)
    result = {"action": args.action, "branch": args.branch}
    ...
    outside = approval_scope.mismatch(scope, args.action, args.branch, datetime.now(timezone.utc))
    if outside:
